...
public class AddModeratorServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request,
        ...
        if (null == account || !PrivilegeUtil.checkAdmin(account.getAccount_limit())) {
            ...
            acc2Add.setAccount_name(request.getParameter("moderator_name"));
            acc2Add = accountDaoConn.returnAccountByAccount_name(acc2Add);
            if (accountDaoConn.setAccountAdmin(acc2Add, Integer.parseInt(
                    request.getParameter("cata_id")))) {
                response.sendRedirect("Admin.jsp");
            } else {
                response.sendRedirect("Admin.jsp?flag=3");
            }
        }
    }
