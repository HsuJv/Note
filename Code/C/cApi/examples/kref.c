...
void print(Table_T files) {
    ...
    for (i = 0; array[i]; i += 2) {
        ...
        if (*filename) Fmt_print("\t%S:", filename, 1, 0);
        while (Seq_length(seq) > 0) Fmt_print(" %S", Seq_remhi(seq), 1, 0);
        Fmt_print("\n");
        FREE(filename);
        Seq_free(&seq);
    }
...
void kref(char *name, FILE *fp, Table_T identifiers) {
    ...
    for (linenum = 1; fgets(buf, sizeof buf, fp) != NULL; linenum++) {
        ...
        for (i = 0; (j = getword(buf, &i, first, rest)) > 0; i = j) {
            ...
            if (files == NULL) {
                files = Table_new(
                    0, (int (*)(const void *, const void *))strcmp, strhash);
...
int main(int argc, char *argv[]) {
    int i;
    Table_T identifiers =
        Table_new(5000, (int (*)(const void *, const void *))strcmp, strhash);
    ...
    if (argc == 1) kref(NULL, stdin, identifiers);
    {
        int i;
