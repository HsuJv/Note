...
Text_T getword(Text_T *line, Text_T first, Text_T rest) {
    ...
    if ((i = Text_rupto(*line, 1, 0, first)) > 0) {
        Text_T word =
            Text_sub(*line, j = Text_rmany(*line, 1, i + 1, rest), i + 1);
...
unsigned texthash(const void *x) {
    ...
    for (i = 0; i < t->len; i++) h = (h << 1) + t->str[i];
    return h;
}
...
void print(Table_T files) {
    ...
    for (i = 0; array[i]; i += 2) {
        ...
        if (filename->len > 0) Fmt_print("\t%T:", filename);
...
void iref(char *name, FILE *fp, Table_T identifiers) {
    ...
    if (name) filename = Text_put(name);
    ...
    while (fgets(buf, sizeof buf, fp) != NULL) {
        ...
        while ((id = getword(&line, first, rest)).len > 0) {
            ...
            } else if (MP_cmp(Array_get(array, Array_length(array) - 1),
                              linenum) != 0) {
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 1) iref(NULL, stdin, identifiers);
    {
        int i;
