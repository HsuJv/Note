...
#define maxdigits(x, y) \
    ((x)->ndigits > (y)->ndigits ? (x)->ndigits : (y)->ndigits)
...
static T add(T z, T x, T y) {
    ...
    if (x->ndigits < n)
        ...
        z->digits[z->size - 1] =
            XP_sum(x->ndigits - n, &z->digits[n], &x->digits[n], carry);
    } else
...
T AP_div(T x, T y) {
    ...
    r = mk(y->ndigits);
    {
        ...
        XP_div(x->ndigits, q->digits, x->digits, y->ndigits, y->digits,
               r->digits, tmp);
        FREE(tmp);
    }
    normalize(q, q->size);
...
T AP_pow(T x, T y, T p) {
    ...
    if (isone(x)) return AP_new((((y)->digits[0] & 1) == 0) ? 1 : x->sign);
    if (p)
        if (isone(y))
            z = AP_mod(x, p);
        else {
