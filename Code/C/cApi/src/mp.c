...
T MP_new(unsigned long u) { return MP_fromintu(ALLOC(nbytes), u); }
...
T MP_cvt(int m, T z, T x) {
    ...
    if (m < nbits) {
        ...
        for (i = mbytes; i < nbytes; i++) carry |= x[i] ^ fill;
        memcpy(z, x, mbytes);
        z[mbytes - 1] &= ones(m);
        if (carry) RAISE(MP_Overflow);
    } else {
        memcpy(z, x, nbytes);
        z[nbytes - 1] |= fill & ~msb;
        for (i = nbytes; i < mbytes; i++) z[i] = fill;
        z[mbytes - 1] &= ones(m);
    }
    return z;
}
...
T MP_cvtu(int m, T z, T x) {
    ...
    if (m < nbits) {
        ...
        for (i = mbytes; i < nbytes; i++) carry |= x[i];
        memcpy(z, x, mbytes);
        z[mbytes - 1] &= ones(m);
        if (carry) RAISE(MP_Overflow);
    } else {
        memcpy(z, x, nbytes);
        for (i = nbytes; i < mbytes; i++) z[i] = 0;
    }
    return z;
}
...
T MP_mulu(T z, T x, T y) {
    ...
    {
        int i;
        if (tmp[3][nbytes - 1] & ~msb) RAISE(MP_Overflow);
        for (i = 0; i < nbytes; i++)
...
T MP_add(T z, T x, T y) {
    ...
    z[nbytes - 1] &= msb;
    if (sx == sy && sy != sign(z)) RAISE(MP_Overflow);
    return z;
}
...
T MP_neg(T z, T x) {
    ...
    XP_neg(nbytes, z, x, 1);
    z[nbytes - 1] &= msb;
    if (sx && sx == sign(z)) RAISE(MP_Overflow);
    return z;
}
...
T MP_mul(T z, T x, T y) {
    ...
    if (sx == sy && sign(z)) RAISE(MP_Overflow);
    return z;
}
T MP_div(T z, T x, T y) {
    ...
    if (sx != sy) {
        XP_neg(nbytes, z, z, 1);
        if (!iszero(tmp[2])) XP_diff(nbytes, z, z, 1);
        z[nbytes - 1] &= msb;
    } else if (sx && sign(z))
        RAISE(MP_Overflow);
    return z;
...
T MP_mului(T z, T x, unsigned long y) {
    ...
    if (y < BASE) {
        ...
        z[nbytes - 1] &= msb;
        if (carry) RAISE(MP_Overflow);
        if (nbits < 8 && y >= (1U << nbits)) RAISE(MP_Overflow);
...
void MP_fmtu(int code, va_list_box *box, int put(int c, void *cl), void *cl,
...
void MP_fmt(int code, va_list_box *box, int put(int c, void *cl), void *cl,
            unsigned char flags[], int width, int precision) {
    T x;
    int base, size, sx;
    char *buf;
