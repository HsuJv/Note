...
void Bit_set(T set, int lo, int hi) {
    ...
    if (lo / 8 < hi / 8) {
        ...
        {
            int i;
            for (i = lo / 8 + 1; i < hi / 8; i++) set->bytes[i] = 0xFF;
        }
...
T Bit_union(T s, T t){setop(copy(t), copy(t), copy(s), |)} T
    Bit_inter(T s,
              T t){setop(copy(t), Bit_new(t->length), Bit_new(s->length), &)} T
    Bit_minus(T s, T t){setop(Bit_new(s->length), Bit_new(t->length), copy(s),
                              &~)} T Bit_diff(T s, T t) {
