...
static int interrupt(int sig, struct sigcontext sc) {
    if (critical || sc.eip >= (unsigned long)_MONITOR &&
...
static int interrupt(int sig, int code, struct sigcontext *scp) {
    if (critical || scp->sc_pc >= (unsigned long)_MONITOR &&
    ...
    if (preempt) {
        {
            ...
            if (sigaction(SIGVTALRM, &sa, NULL) < 0) return 0;
        }
        {
            struct itimerval it;
            ...
            if (setitimer(ITIMER_VIRTUAL, &it, NULL) < 0) return 0;
        }
    }
    return 1;
}
