...
T Text_map(T s, const T *from, const T *to) {
    ...
    if (s.len == 0)
        ...
        text.len = s.len;
        text.str = p = alloc(s.len);
        for (i = 0; i < s.len; i++) *p++ = map[(unsigned char)s.str[i]];
        return text;
    }
}
...
int Text_any(T s, int i, T set) {
    ...
    i = idx(i, s.len);
    assert(i >= 0 && i <= s.len);
    if (i < s.len && memchr(set.str, s.str[i], set.len)) return i + 2;
    return 0;
}
...
void Text_fmt(int code, va_list_box *box, int put(int c, void *cl), void *cl,
