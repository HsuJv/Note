...
#define xx(t, v) \
    if ((char *)&v.v - &v.pad > max) max = (char *)&v.v - &v.pad
    yy
#undef xx
    ...
    char *ptr = malloc(1);
    if (verbose) fprintf(stderr, "malloc(1) = %p\n", ptr);
    while (max > 0 && ((unsigned)ptr) % max != 0) max /= 2;
    assert(max);

    if (max != sizeof(union align)) printf("-DMAXALIGN=%u\n", max);
    return EXIT_SUCCESS;
}
