 ...
 *
 * *********************************************************************
 * Attaching the original author's comments:
 *
 * J. David's webserver
 ...
 *
 * *********************************************************************
 * Chinese Comments' author：
 *      github: cbsheng
 * see (https://github.com/cbsheng/tinyhttpd)
 * *********************************************************************
 */
...
#define DEFAUTL_HTTP_ROOT "./http"
#define ENV_ROOT "DPI_ROOT_PATH"
...
static void accept_request(int client) {
    ...
    sprintf(path, "%s%s", _root_path ? _root_path : DEFAUTL_HTTP_ROOT, url);
...
static void cat(int client, FILE *resource) {
    ...
    while (!feof(resource)) {
        ...
        fgets(buf, sizeof(buf) - 2, resource);
    }
...
/**********************************************************************/
static void execute_cgi(int client, const char *path, const char *method,
    ...
    if (pid == 0) /* child: CGI script */
    {
        ...
        if (strcasecmp(method, "POST") == 0)
            for (i = 0; i < ceil(content_length / readlen); i++) {
                recv(client, buf, readlen, 0);
                write(cgi_input[1], buf, readlen);
            }

        //然后从 cgi_output 管道中读子进程的输出，并发送到客户端去
        while (read(cgi_output[0], buf, readlen) > 0) {
            send(client, buf, strlen(buf), 0);
        }
...
static int startup(u_short *port) {
    ...
    int set = 1;
    // sockaddr_in 是
    // IPV4的套接字地址结构。定义在<netinet/in.h>,参读《TLPI》P1202
    struct sockaddr_in name;

    // socket()用于创建一个用于 socket
    // 的描述符，函数包含于<sys/socket.h>。参读《TLPI》P1153
    //这里的PF_INET其实是与 AF_INET同义，具体可以参读《TLPI》P946
    httpd = socket(PF_INET, SOCK_STREAM, 0);
    if (setsockopt(httpd, SOL_SOCKET, SO_REUSEPORT, &set, sizeof(int)) < -1)
        error_die("reuse port");
...
int runHttpdBkg(void *arg) {
    prctl(PR_SET_NAME, "HTTP SERVER");

    // detach it self
    pthread_detach(pthread_self());

    // change work dir
    char *work_root = getenv(ENV_ROOT);
    if (work_root) chdir(work_root);
    ...
    while (1) {
        ...
        // set nopipe
        int set = 1;
        setsockopt(client_sock, SOL_SOCKET, MSG_NOSIGNAL, (void *)&set,
                   sizeof(int));
