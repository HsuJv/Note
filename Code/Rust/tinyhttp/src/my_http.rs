...
use crate::my_cgi::{cgi_exec, CgiAction};
use bytes::BytesMut;
use futures::SinkExt;
use http::{header::HeaderValue, method::Method, Request, Response, StatusCode};
use std::{env, error::Error, fmt, fs, io};
use tokio::net::{TcpListener, TcpStream};
use tokio::stream::StreamExt;
use tokio_util::codec::{Decoder, Encoder, Framed};

pub async fn run_http_server(ip: &str, port: &str) -> Result<(), Box<dyn Error>> {
    let addr: String = String::from(ip) + &String::from(":") + &String::from(port);
...
fn send_file(file_name: &str) -> (StatusCode, String) {
    let file_path = env::var("File_Root").unwrap() + &file_name;

    if let Ok(ctx) = fs::read_to_string(file_path) {
        return (StatusCode::OK, ctx);
    } else {
        match send_file("/404.html") {
            (_, ctx) => return (StatusCode::NOT_FOUND, ctx),
        }
    }
}

fn redirect_to(file_name: &str) -> (StatusCode, String) {
    (StatusCode::TEMPORARY_REDIRECT, file_name.to_string())
}
...
async fn respond(req: Request<()>) -> Result<Response<String>, Box<dyn Error>> {
    ...
    let (code, body) = match req.method() {
        &Method::GET => {
            ...
            match req.uri().path() {
                ...
                path if "/cgi/restart" == path => {
                    cgi_exec(CgiAction::Restart, req.uri().query()).await?;
                    redirect_to("/restarting.html")
                }
                path if "/cgi/sync_job" == path => {
                    cgi_exec(CgiAction::SyncJob, req.uri().query()).await?;
                    redirect_to("/index.html")
                }
                path if "/cgi/get_emails" == path => {
                    let email_list = cgi_exec(CgiAction::GetEmails, req.uri().query()).await?;
                    (StatusCode::OK, email_list)
                }
                path if "/cgi/add_email" == path => {
                    cgi_exec(CgiAction::AddEmail, req.uri().query()).await?;
                    redirect_to("/cgi/restart")
                }
                path if "/cgi/get_users" == path => {
                    let user_list = cgi_exec(CgiAction::GetUsers, req.uri().query()).await?;
                    (StatusCode::OK, user_list)
                }
                path if "/cgi/add_user" == path => {
                    cgi_exec(CgiAction::AddUser, req.uri().query()).await?;
                    redirect_to("/cgi/restart")
                }
    ...
    if StatusCode::TEMPORARY_REDIRECT == code {
