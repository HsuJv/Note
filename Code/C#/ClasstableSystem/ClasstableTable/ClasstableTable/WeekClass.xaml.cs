...
namespace ClasstableTable
{
    ...
    /// </summary>
    public sealed partial class WeekClass : Page
    {
        string usrname;
        int auth;
        ...
        private void comboBoxChooseeek_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            int CurWeek = comboBoxChooseeek.SelectedIndex + 1;
            ApplicationDataContainer localSettings = ApplicationData.Current.LocalSettings;
            if (localSettings.Values[usrname+"ClassNum"] != null)
            {
                ...
                for (int i = 0; i < lastIndex; i++)
                {
                    ...
                    if (DuelOrSingle == 2) DuelOrSingle = 0;
                    if (StartWeek <= CurWeek && CurWeek <= EndWeek && (DuelOrSingle == 3 || CurWeek % 2 == DuelOrSingle))
                    {
                        foreach (var t in grd.Children)
                        {
                            if (t is TextBox)
                            {
                                TextBox tbox = t as TextBox;
                                if (String.IsNullOrEmpty(tbox.Text) && Grid.GetColumn(tbox) == WeekTime && Grid.GetRow(tbox) == ClassTime + 1)
                                {
                                    tbox.Text = CourseName + "@" + TeacherName + "(" + ClassRoom + ")";
                                }
                            }
                        }
                    }
